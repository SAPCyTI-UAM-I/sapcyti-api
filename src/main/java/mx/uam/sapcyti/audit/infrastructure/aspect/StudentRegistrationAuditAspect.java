...
@Slf4j
public class StudentRegistrationAuditAspect {

    private final AuditOutputPort auditOutputPort;

    @Pointcut("execution(* mx.uam.sapcyti.academic.application.service.RegisterStudentUseCase.execute(..))")
    public void registerStudentPointcut() {}

    @Pointcut("execution(* mx.uam.sapcyti.academic.infrastructure.adapter.in.StudentController.*(..))")
    public void studentControllerPointcut() {}

    @AfterReturning(pointcut = "registerStudentPointcut()", returning = "result")
    public void auditStudentRegistered(Object result) {
        if (!(result instanceof RegisterStudentUseCase.RegisterStudentResult registered)) {
            return;
        }

        recordEvent(
                KnownAuditActions.STUDENT_REGISTERED.name(),
                resolveActorId(),
                resolveActorRole(),
                registered.getGraduateProgramId(),
                AuditSeverityLevel.STANDARD,
                "studentId=%d,userId=%d,enrollmentId=%s".formatted(
                        registered.getId(),
                        registered.getUserId(),
                        registered.getEnrollmentId()));
    }

    @AfterThrowing(pointcut = "studentControllerPointcut()", throwing = "ex")
    public void auditStudentAccessDenied(AccessDeniedException ex) {
        recordEvent(
