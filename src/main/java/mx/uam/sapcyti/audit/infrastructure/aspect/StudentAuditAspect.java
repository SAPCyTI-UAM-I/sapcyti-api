...
public class StudentAuditAspect {
    ...
    @Pointcut("execution(* mx.uam.sapcyti.academic.application.service.UpdateStudentUseCase.execute(..))")
    public void updateStudentPointcut() {}

    @AfterReturning(pointcut = "updateStudentPointcut()", returning = "result")
    public void auditStudentUpdated(Object result) {
        if (!(result instanceof ListStudentsUseCase.StudentListItem updated)) {
            return;
        }

        recordEvent(
                KnownAuditActions.STUDENT_UPDATED.name(),
                resolveActorId(),
                resolveActorRole(),
                updated.getGraduateProgramId(),
                AuditSeverityLevel.STANDARD,
                "studentId=%d,email=%s,active=%s".formatted(
                        updated.getId(), updated.getEmail(), updated.isActive()));
    }
